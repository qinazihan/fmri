...
for task_idx in range(len(tasks_list)):
    ...
    for model in model_list:
        ...
        for state in brainstates:
            ...
            CORR_Df = pd.DataFrame(np.hstack((np.array(R2_score).reshape(-1,1),np.array(R2_score_TIME).reshape(-1,1),np.array(CORR).reshape(-1,1),np.array(CORR_TIME).reshape(-1,1))),columns = ['R2_Score','R2_Score_Time','Corr','Corr_Time'])
            ...
            np.save('stats_dict_relative_time.npy',stats_dict)
